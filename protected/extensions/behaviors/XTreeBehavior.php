...
class XTreeBehavior extends CActiveRecordBehavior
{
	...
	/**	 
	 * key-value pair that can be used in a conditional statement to find the root element
	 * @var array
	 */
	public $root_condition = array('parent'=>'top');	
	...
	{
		$owner=$this->getOwner();		
		$root=$owner->findByAttributes($this->root_condition);		
		return $root->{$this->parent_id};
	}
	...
	public function getTreeItems($id=null,$showRoot=true)
	{
		...
		else
		{
			...
			$model=$owner->findByAttributes(array("{$this->parent_id}"=>$rootId));					
			if($model===null)
				throw new CException('The requested tree does not exist.');
			else
				$items[]=$model->getTreeSubItems();				
